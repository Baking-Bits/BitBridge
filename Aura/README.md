...
You can also build Aura with PlatformIO.

1. Open this `Aura` folder in VS Code.
2. Install PlatformIO IDE extension.
3. Open `platformio.ini` and confirm environment `aura_cyd`.
4. Run **PlatformIO: Build**.
5. Connect board and run **PlatformIO: Upload**.

Notes:

- This repo already includes `platformio.ini` configured for ESP32 Dev Module + huge app partition.
- The project uses local config headers in `include/User_Setup.h` and `include/lv_conf.h` that point to the existing repo files.
